...
- (void)presentViewController:(UIViewController *)viewControllerToPresent animated:(BOOL)flag completion:(void (^)(void))completion
{
    ...
    if ([self.viewControllers count] > 1)
    {
        ...
        {
            nav.topViewController.navigationItem.leftBarButtonItem = [self navBarItemWithTarget:self action:@selector(dismissLeftBtnPress:) imageName:@"icon_return.png"];
        }
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated
{
    ...
    if ([self.viewControllers count] > 1)
    {
        ...
        {
            viewController.navigationItem.leftBarButtonItem = [self navBarItemWithTarget:self action:@selector(backBtnPressed:) imageName:@"icon_return.png"];
        }
        
        if (viewController.navigationItem.rightBarButtonItem == nil)
        {
            viewController.navigationItem.rightBarButtonItem = [self navBarItemWithTarget:self action:@selector(rightBtnPressed:) imageName:@"icon_personal.png"];
        }
