...
#define UICCALENDAR_TITLE_FONT_SIZE 17.0f
...
#define UICCALENDAR_CELL_EXTRALARGE_WIDTH 46.0f
#define UICCALENDAR_CELL_EXTRALARGE_HEIGHT 36.0f
...
+ (void)initialize {
	...
	disabledCell = [[UIImage imageNamed:@"uiccalendar_cell_normal.png"] retain];
	monthoutCell = [[UIImage imageNamed:@"uiccalendar_cell_normal.png"] retain];
	...
	selectedColor = [[UIColor lightGrayColor] retain];
	disabledColor = [[UIColor lightGrayColor] retain];
	monthoutColor = [[UIColor whiteColor] retain];
	holidayColor  = [[UIColor lightGrayColor] retain];
}
...
- (id)initWithSize:(UICCalendarPickerSize)viewSize {
	LOG_CURRENT_METHOD;
	if (self = [super initWithFrame:CGRectMake(0.0f, 0.0f, cellWidth[viewSize] * 7 - 6.0f + 21.0f, cellHeight[viewSize] * 6 - 5.0f + 82.0f-36.0f)]) {
		self.titleText = @"Calendar";
		self.weekText = [NSArray arrayWithObjects:@"周日", @"周一", @"周二", @"周三", @"周四", @"周五", @"周六", nil];
		...
		[prevButton setFrame:CGRectMake(16.0f, 36.0f - 34.0f, 20,20)];
		...
		[nextButton setFrame:CGRectMake(self.frame.size.width - 41.0f - viewSize, 36.0f-34.0f, 20,20)];
		...
		[monthLabel setFrame:CGRectMake(self.frame.size.width / 2 - UICCALENDAR_TITLE_LABEL_WIDTH / 2, viewSize, UICCALENDAR_TITLE_LABEL_WIDTH, UICCALENDAR_TITLE_LABEL_HEIGHT)];
		...
		for (int i = 0; i < 7; i++) {
			...
			[weekLabel setFont:[UIFont fontWithName:@"Arial-BoldMT" size:13]];
			[weekLabel setFrame:CGRectMake(10.0f + (cellWidth[viewSize] - 1) * (i % 7), 63.0f-36.0f, cellWidth[viewSize] , UICCALENDAR_TITLE_LABEL_HEIGHT)];
		...
		for (int i = 0; i < 42; i++) {
			...
			[dateButton setFrame:
			 CGRectMake(9.0f + cellWidth[viewSize] * (i % 7) - (i % 7), 82.0f - 36.0f + cellHeight[viewSize] * (i / 7) - (i / 7), cellWidth[viewSize]-1, cellHeight[viewSize]-1)];
        ...
        //NSDateComponents封装了具体年月日、时秒分等信息
...
- (void)picker:(UICCalendarPicker *)picker buttonForDate:(UICCalendarPickerDateButton *)button {
    ...
    if ([dataSource respondsToSelector:@selector(picker:buttonForDate:)]) {
		[dataSource picker:self buttonForDate:button];
	}
...
//		self.frame = frame;
        ...
        self.frame = CGRectMake(frame.origin.x, 51, frame.size.width, frame.size.height);
...
/*
 dateButton设置日期的地方应该是没有问题，但是使用的日历是有问题的，不是中国的时间
 */
- (void)dateButtonPushed:(id)sender {
	@synchronized(self) {
		...
		if (selectionMode == UICCalendarPickerSelectionModeSingleSelection) {
            NSLog(@"SelectedDate$$$$$$$$$$$= %@", selectedDates);
...
//		LOG(@"SelectedDate = %@", selectedDates);
        NSLog(@"SelectedDate *********= %@", selectedDates);
...
- (void)setUpCalendarWithDate:(NSDate *)aDate {
	LOG(@"setUpCalendarWithDate = %@", aDate);
